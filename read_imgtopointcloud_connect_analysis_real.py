...
for i in range(width):
     for j in range(height):
         if ele_image[i, j,0]<253:
            raw_point_cloud_matrix.append([i*scale_h,j*scale_w,ele_image[i, j,0] / 255 * 50])
...
for i in range(points.shape[0]):
    ...
    point = [point_cloud_o3d.points[i], point_cloud_o3d.points[i]+v[:, 0]*line_size, point_cloud_o3d.points[i]+v[:, 1]*line_size, point_cloud_o3d.points[i]+v[:, 2]*line_size]
    lines = [[0, 1], [0, 2],[0, 3]]
    colors = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
    mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size=10, origin=point_cloud_o3d.points[i])
    line_set = o3d.geometry.LineSet(points=o3d.utility.Vector3dVector(point), lines=o3d.utility.Vector2iVector(lines))
    line_set.colors = o3d.utility.Vector3dVector(colors)
